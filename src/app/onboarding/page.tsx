  ...
  return (
    <div className="max-w-4xl w-full mx-auto fade-in animate-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-16">
        ...
        {/* Something Else */}
        <div className="md:col-span-8 group cursor-pointer" onClick={() => handleSelect("other")}>
          <div className={`relative overflow-hidden h-64 rounded-xl border transition-all hover:-translate-y-1 flex items-center justify-center bg-surface-container-low ${niche === "other" ? "border-primary ring-2 ring-primary bg-primary/5" : "border-outline-variant/15 shadow-[0_12px_32px_-4px_rgba(67,82,165,0.06)]"}`}>
            <div className="text-center p-8">
              <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full mb-4 ${niche === "other" ? "bg-primary text-white" : "bg-surface-container-lowest text-primary"}`}>
                <span className="material-symbols-outlined text-3xl">add</span>
              </div>
              <h3 className="text-xl font-headline font-bold text-on-surface tracking-tight">¿Algo más?</h3>
              <p className="text-on-surface-variant text-sm">Escribe tu propio nicho creativo.</p>
            </div>
          </div>
        </div>
      </div>
...
function Step3ConnectSocials() {
  ...
  const { connectedSocials, toggleSocial, slug, bio, prevStep } = useOnboardingStore();
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      toast({ title: "Bienvenida a Pina", description: "Tu estudio está listo." });
  ...
  return (
    <div className="bg-surface font-body text-on-surface antialiased overflow-x-hidden min-h-screen relative">
      <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-8 h-20 bg-surface/70 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-4">
            <button className="text-on-surface-variant text-sm font-medium hover:text-primary transition-colors">Guardar y Salir</button>
        </div>
      </header>
